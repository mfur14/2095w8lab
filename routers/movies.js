...
module.exports = {                      // in Java terms, we are creating a class
    ...
    addActor: function(req, res){
        ...
        Movie.findById(movId, function(err, movie){ 
            Actor.findById(actId, function(err, actor){
    ...
    year: function(req, res){
        let yOne = req.params.year1;
        let yTwo = req.params.year2;

        Movie.find({year: {$gte: yTwo, $lte: yOne}}, function(err, data){
            if (err) return res.status(400).json(err);
            res.json(data);
        });
    }
